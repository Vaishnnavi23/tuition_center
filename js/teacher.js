...
async function loadClasses() {
  const { data: classes, error } = await sb.from("classes").select("*").eq("teacher_id", TEACHER_PROFILE.id);
...
async function loadStudents() {
  const { data: students, error } = await sb.from("profiles").select("*").eq("teacher_id", TEACHER_PROFILE.id).eq("role", "student");
...
async function markAndGenerate(studentId, classId) {
  ...
  const { error } = await sb.from("attendance").insert({ student_id: studentId, class_id: classId, status, date });
