...
async function loadClasses() {
  const { data: classes, error } = await sb.from("classes").select("*").eq("teacher_id", STUDENT_PROFILE.teacher_id);
...
async function loadAttendance() {
  const { data: records, error } = await sb.from("attendance").select("*").eq("student_id", STUDENT_PROFILE.id).order("date", { ascending: false });
...
async function loadHomework() {
  const { data: classes } = await sb.from("classes").select("id").eq("teacher_id", STUDENT_PROFILE.teacher_id);
  ...
  const { data: hw, error } = await sb.from("homework").select("*").in("class_id", classIds).order("posted_at", { ascending: false });
