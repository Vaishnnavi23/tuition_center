...
async function loadStats() {
  const { data: users } = await sb.from("profiles").select("role");
  const { data: classes } = await sb.from("classes").select("id");
...
async function loadClasses() {
  const { data: classes, error } = await sb.from("classes").select("*, profiles!classes_teacher_id_fkey(name)");
